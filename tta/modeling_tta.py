...
class TTASelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(
        ...
        self.query_hidden = mixed_query_layer
...
class TTAAttention(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.output(self_outputs[0], self.self.query_hidden)
        outputs = (attention_output,) + self_outputs[1:]  # add attentions if we output them
        return outputs


class ttaLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class TTAEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.value = nn.Linear(config.hidden_size, self.all_head_size)
        self.key = nn.Linear(config.hidden_size, self.all_head_size)
        self.layer = nn.ModuleList([ttaLayer(config) for _ in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        key_layer = self.transpose_for_scores(self.key(combined_hidden))
        value_layer = self.transpose_for_scores(self.value(combined_hidden))
...
class TTAModel(BertModel):
    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
        self.config = config

        self.embeddings = TTAEmbeddings(config)
        self.encoder = TTAEncoder(config)
    ...
    def forward(
        ...
        encoder_outputs = self.encoder(
            embedding_output, combined_hidden=combined_embedding,
...
class TTALMModel(nn.Module):

    def __init__(self, config):
        super().__init__()
        self.tta = TTAModel.from_pretrained(config, add_pooling_layer=False)
        self.cls = BertLMPredictionHead(self.tta.config)
        self.config = self.tta.config
